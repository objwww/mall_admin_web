...
import {
  getLimitDiscountListAPI, createLimitDiscountAPI, updateLimitDiscountAPI, deleteLimitDiscountAPI,
  getLimitDiscountProductListAPI, createLimitDiscountProductAPI, deleteLimitDiscountProductAPI,
} from '@/apis/marketing'
...
const getList = async () => {
  loading.value = true
  try {
    const res = await getLimitDiscountListAPI({ ...listQuery })
...
const formRef = ref()
const form = reactive<Record<string, any>>({ id: null, name: '', startTime: null as number | null, endTime: null as number | null, status: 0 })
const rules = { name: [{ required: true, message: '请输入活动名称', trigger: 'blur' }] }

const openDialog = (row?: Record<string, any>) => {
  Object.assign(form, row
    ? { id: row.id, name: row.name, startTime: row.start_time, endTime: row.end_time, status: row.status }
    : { id: null, name: '', startTime: null, endTime: null, status: 0 })
  dialogVisible.value = true
}

const submit = async () => {
  await formRef.value.validate()
  const params = { name: form.name, startTime: form.startTime, endTime: form.endTime, status: form.status }
  if (form.id) {
    await updateLimitDiscountAPI({ id: form.id, ...params })
    ElMessage.success('修改成功')
  } else {
    await createLimitDiscountAPI(params)
...
const handleDelete = async (row: Record<string, any>) => {
  ...
  await deleteLimitDiscountAPI(row.id)
...
const openProducts = async (row: Record<string, any>) => {
  ...
  try {
    const res = await getLimitDiscountProductListAPI(row.id)
...
const productForm = reactive<Record<string, any>>({ productId: null, productName: '', originalPrice: 0, discountPrice: 0, discountRate: 1, sort: 0 })
const productDialogVisible = ref(false)

const openProductDialog = () => {
  Object.assign(productForm, { productId: null, productName: '', originalPrice: 0, discountPrice: 0, discountRate: 1, sort: 0 })
...
const submitProduct = async () => {
  ...
  await createLimitDiscountProductAPI({ activityId: currentActivity.value!.id, ...productForm })
...
const handleDeleteProduct = async (row: Record<string, any>) => {
  ...
  await deleteLimitDiscountProductAPI(row.id)
  ...
  <div class="app-container">
    <el-card shadow="never" class="filter-container">
      <div class="card-header">
        <span class="card-title">限时折扣</span>
    ...
    <!-- 活动商品弹窗 -->
    <el-dialog v-model="productVisible" :title="'活动商品：' + (currentActivity?.name || '')" width="760px" destroy-on-close>
      <div class="table-toolbar">
        <span class="toolbar-title">商品列表</span>
        <el-button type="primary" size="small" :icon="Plus" @click="openProductDialog">添加商品</el-button>
      </div>
      <el-table :data="productList" v-loading="productLoading" border size="small">
        ...
        <el-table-column label="折扣价" width="90" align="center">
          <template #default="scope">
            <span class="color-danger">￥{{ scope.row.discount_price }}</span>
          </template>
        </el-table-column>
        <el-table-column label="折扣率" width="80" align="center">
          <template #default="scope">{{ scope.row.discount_rate }}</template>
        </el-table-column>
    ...
    <el-dialog v-model="productDialogVisible" title="添加活动商品" width="520px" destroy-on-close>
      <el-form :model="productForm" label-width="90px">
        ...
        <el-form-item label="原价(元)">
          <el-input-number v-model="productForm.originalPrice" :min="0" :precision="2" style="width: 200px" />
        </el-form-item>
        <el-form-item label="折扣价(元)">
          <el-input-number v-model="productForm.discountPrice" :min="0" :precision="2" style="width: 200px" />
        </el-form-item>
        <el-form-item label="折扣率">
          <el-input-number v-model="productForm.discountRate" :min="0.01" :max="1" :step="0.05" :precision="2" style="width: 200px" />
        </el-form-item>
