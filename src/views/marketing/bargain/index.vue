...
import {
  getBargainListAPI, createBargainAPI, updateBargainAPI, deleteBargainAPI,
  getBargainProductListAPI, createBargainProductAPI, deleteBargainProductAPI,
} from '@/apis/marketing'
...
const getList = async () => {
  loading.value = true
  try {
    const res = await getBargainListAPI({ ...listQuery })
...
const submit = async () => {
  ...
  if (form.id) {
    await updateBargainAPI({ id: form.id, ...params })
    ElMessage.success('修改成功')
  } else {
    await createBargainAPI(params)
...
const openProducts = async (row: Record<string, any>) => {
  ...
  try {
    const res = await getBargainProductListAPI(row.id)
...
const productForm = reactive<Record<string, any>>({ productId: null, productName: '', floorPrice: 0, originalPrice: 0, stock: 0, sort: 0 })
...
const submitProduct = async () => {
  ...
  await createBargainProductAPI({ activityId: currentActivity.value!.id, ...productForm })
...
const handleDeleteProduct = async (row: Record<string, any>) => {
  ...
  await deleteBargainProductAPI(row.id)
  ...
  <div class="app-container">
    ...
    <el-dialog v-model="productVisible" :title="'活动商品：' + (currentActivity?.name || '')" width="720px" destroy-on-close>
      ...
      <el-table :data="productList" v-loading="productLoading" border size="small">
        ...
        <el-table-column label="商品名称" prop="product_name" min-width="150" show-overflow-tooltip />
        <el-table-column label="原价" width="90" align="center">
          <template #default="scope">￥{{ scope.row.original_price }}</template>
        </el-table-column>
        <el-table-column label="底价" width="90" align="center">
          <template #default="scope">
            <span class="color-danger">￥{{ scope.row.floor_price }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="productDialogVisible" title="添加活动商品" width="520px" destroy-on-close>
      <el-form :model="productForm" label-width="90px">
        ...
        <el-form-item label="底价(元)">
          <el-input-number v-model="productForm.floorPrice" :min="0" :precision="2" style="width: 200px" />
        </el-form-item>
        <el-form-item label="砍价库存">
