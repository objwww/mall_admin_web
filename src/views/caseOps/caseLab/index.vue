...
const form = reactive({ familyId: 'REF-002', refundId: '', ttlSeconds: 60, maxAffectedRequests: 1 })
...
const createSession = async () => {
  if (!/^[1-9][0-9]*$/.test(form.refundId)) return ElMessage.warning('退款单编号必须为正整数')
  ...
  const { data } = await createCaseSessionAPI({
    ...
    maxAffectedRequests: form.maxAffectedRequests, scope: { refundId: form.refundId },
  })
...
<template>
  <div class="app-container">
    <el-alert title="仅限隔离测试环境" type="error" :closable="false" show-icon description="生产环境不得开启 CaseLab。Agent 与商城端看不到故障点、动作、Oracle 和触发证据。" />
    <el-card v-for="family in families" :key="family.familyId" shadow="never" class="family-card">
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="隐藏 Oracle">{{ family.oracle }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
    <el-card shadow="never">
      <template #header><div class="header"><strong>注入会话</strong><div><el-button @click="load">刷新</el-button><el-button type="danger" @click="dialogVisible = true">创建会话</el-button></div></div></template>
      <el-table v-loading="loading" :data="sessions" border stripe>
        ...
        <el-table-column label="作用域" min-width="150"><template #default="scope">退款单：{{ scope.row.scope?.refundId || '-' }}</template></el-table-column>
        ...
        <el-table-column label="操作" width="230" fixed="right"><template #default="scope"><el-button link type="primary" @click="showEvents(scope.row)">触发证据</el-button><el-button link type="primary" @click="createTicket(scope.row)">生成工单</el-button><el-button v-if="scope.row.status === '运行中'" link type="danger" @click="stop(scope.row)">停止</el-button></template></el-table-column>
      </el-table>
    </el-card>
    ...
    <el-dialog v-model="dialogVisible" title="创建故障注入会话" width="520px">
      <el-form label-width="120px">
        ...
        <el-form-item label="退款单编号"><el-input v-model="form.refundId" placeholder="请输入隔离环境退款单ID" /></el-form-item>
